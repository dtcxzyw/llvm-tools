...
int main(int argc, char **argv) {
  ...
  LLVMContext Context;
  std::unordered_map<Intrinsic::ID, uint32_t> Dist;
  uint32_t Count = 0;
    ...
    for (auto &F : *M) {
      ...
      for (auto &BB : F) {
        ...
        if (auto *SI = dyn_cast<SwitchInst>(Terminator)) {
          ...
          if (auto *II = dyn_cast<IntrinsicInst>(Cond)) {
            switch (II->getIntrinsicID()) {
            case Intrinsic::smin:
            case Intrinsic::smax:
            case Intrinsic::umin:
            case Intrinsic::umax: {
              if (auto *RHSC = dyn_cast<ConstantInt>(II->getArgOperand(1))) {
                if (SI->findCaseValue(RHSC)->getCaseIndex() !=
                    SwitchInst::DefaultPseudoIndex) {
                  Dist[II->getIntrinsicID()]++;
                }
              }
              break;
            }
  ...
  errs() << "Count: " << Dist.size() << '\n';
  for (auto &[ID, Count] : Dist)
    errs() << Intrinsic::getBaseName(ID) << ' ' << Count << '\n';
  return EXIT_SUCCESS;
}
