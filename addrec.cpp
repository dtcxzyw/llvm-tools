...
int main(int argc, char **argv) {
  ...
  for (auto &Path : InputFiles) {
    ...
    TargetLibraryInfoImpl TLIImpl{M->getTargetTriple()};
    ...
    for (auto &F : *M) {
      ...
      for (auto &BB : F) {
        for (auto &I : BB) {
          ...
          Follower F;
          SCEVTraversal<Follower> Traversal{F};
          Traversal.visitAll(S);
        }
      }
    }
