...
keyword = sys.argv[2]
csmith_ext = ""
csmith_command = csmith_dir +"/bin/csmith --max-funcs 5 --quiet --builtins --no-packed-struct --no-unions --no-bitfields --no-volatiles --no-volatile-pointers {}--output ".format(
    csmith_ext)
common_opts = "-Wno-narrowing -DNDEBUG -g0 -ffp-contract=on -w -I" + csmith_dir + "/include "
gcc_command = "clang -O3 " + common_opts
...
def run_and_check(basename, ref_output):
    file_out = basename

    try:
        out = subprocess.check_output([file_out], timeout=exec_timeout)
...
def csmith_test(i):
    ...
    try:
        comp_command = clang_command +" -o "+basename+" "+file_c
        out = subprocess.check_output(comp_command.split(' '), timeout=comp_timeout).decode()
        if keyword not in out:
            os.remove(basename)
            os.remove(file_c)
            return None
    except subprocess.SubprocessError:
        with open(basename+"_comp.sh", "w") as f:
    ...
    if not run_and_check(basename, ref_output):
        result = False
