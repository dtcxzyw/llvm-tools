...
csmith_command = (
    csmith_dir
    + "/bin/csmith --max-funcs 3 --max-block-depth 5 --quiet --builtins --no-packed-struct --no-unions --no-bitfields --no-volatiles --no-volatile-pointers {}--output ".format(
        csmith_ext
    )
)
common_opts = (
...
clang_command = (
    "/home/dtcxzyw/WorkSpace/Projects/compilers/LLVM/llvm-build/bin/clang -O3 --target=riscv64-linux-gnu "
    + common_opts
)
clang_arch_list = [
...
def build_and_run(arch, basename, file_c, ref_output):
    ...
    try:
        comp_command = (
            clang_command + "-march=" + arch + " -o " + file_out + " " + file_c
        )
    ...
    try:
        out = subprocess.check_output(
            qemu_command + [file_out], timeout=exec_qemu_timeout
        )
...
with ThreadPoolExecutor(max_workers=16) as p:
    for res in p.map(csmith_test, L):
        ...
        pbar.set_description(
            "Failed: {} Skipped: {}".format(error_count, skipped_count)
        )
